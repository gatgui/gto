...
bool Attrib::isCompatible(const Attrib &rhs) const
{
      ...
      getDim() == rhs.getDim())
  {
    if (isMapped())
    {
      if (rhs.isMapped())
      {
        return (mapping().size() == rhs.mapping().size());
      }
      else
      {
        return (mapping().size() == size_t(rhs.getSize()));
      }
    }
    else
    {
      if (rhs.isMapped())
      {
        return (size_t(getSize()) == rhs.mapping().size());
      }
      else
      {
...
bool Attrib::toHoudini(Gto::Reader *r,
                       Gto::Reader *r0, Attrib *a0,
                       Gto::Reader *r1, Attrib *a1,
                       float blend, Attrib *v, float sceneScale, HAttrib &dst)
{
  if (a0)
  {
    if (a1)
    {
      if (blend <= 0.0001f)
      {
        //std::cout << "Use previous sample? " << (a0 ? "yes" : "no") << std::endl;
        return (isCompatible(*a0) ? a0->toHoudini(r0, sceneScale, dst) : toHoudini(r, sceneScale, dst));
      }
      else if ((1.0f - blend) <= 0.0001f)
      {
        //std::cout << "Use next sample? " << (a1 ? "yes" : "no") << std::endl;
        return (isCompatible(*a1) ? a1->toHoudini(r1, sceneScale, dst) : toHoudini(r, sceneScale, dst));
      }
      else
      {
#if UT_MAJOR_VERSION_INT >= 12
        if ((getType() == Gto::Float || getType() == Gto::Double) &&
            isCompatible(*a0) && isCompatible(*a1))
        {
          if (isMapped())
          {
          ...
          if ((mDim == 3 || mDim == 4) && (mInterp == "point" || mInterp == "hpoint" || mInterp == "vector"))
          {
            // use scene scale
          }
          else
          ...
          //std::cout << "Interpolate sample " << blend << std::endl;
          //std::cout << "Scene scale = " << sceneScale << std::endl;
          
          if (getType() == Gto::Float)
          {
            float w0 = 1.0f - blend;
            float w1 = blend;
            float *v0 = (float*) a0->data();
            float *v1 = (float*) a1->data();
            std::vector<int> *i0 = (a0->isMapped() ? &(a0->mapping()) : 0);
            std::vector<int> *i1 = (a1->isMapped() ? &(a1->mapping()) : 0);
            
            for (size_t i=0, k0=0, k1=0; i<dst.elems.size(); ++i)
            {
              k0 = mDim * (i0 ? i0->at(i) : i);
              k1 = mDim * (i1 ? i1->at(i) : i);
              
              for (int j=0; j<mDim; ++j)
              {
                tpl->set(dst.attrib, dst.elems[i], (mDim < 3 ? sceneScale : 1.0) * (w0 * v0[k0+j] + w1 * v1[k1+j]), j);
              }
            }
          }
          else
          {
            double w0 = double(1.0f - blend);
            double w1 = double(blend);
            double *v0 = (double*) a0->data();
            double *v1 = (double*) a1->data();
            std::vector<int> *i0 = (a0->isMapped() ? &(a0->mapping()) : 0);
            std::vector<int> *i1 = (a1->isMapped() ? &(a1->mapping()) : 0);
...
#endif
        // Do not support interpolation on houdini version earlier than 12.0
        {  
          if (blend <= 0.5f)
          {
            //std::cout << "Use previous sample? " << (a0 ? "yes" : "no") << std::endl;
    ...
    {
      //std::cout << "Use sample? " << (a0 ? "yes" : "no") << std::endl;
      return (isCompatible(*a0) ? a0->toHoudini(r0, sceneScale, dst) : toHoudini(r, sceneScale, dst));
    }
  }
  else
  {
    ...
    if (!isMapped() &&
        v &&
        v->getSize() == getSize() &&
        (getType() == Gto::Float || getType() == Gto::Double) &&
        (getDim() == 3 || getDim() == 4) &&
        (mInterp == "point" || mInterp == "hpoint"))
    {
      // In this case, blend should be the velocity vector weight
      // NOTE: this is not yet implemented anyway
      
      // blend = (currentFrame - thisSampleFrame) / fps
      ...
      //std::cout << "Extrapolate sample " << blend << std::endl;
      //std::cout << "Scene scale = " << sceneScale << std::endl;
      
      if (v->getType() == Gto::Double)
      {
        double *vv = (double*) v->data();
        
        if (getType() == Gto::Double)
        {
          double *av = (double*) data();
          
          for (size_t i=0, k=0, l=0; i<dst.elems.size(); ++i, k+=mDim)
          {
            for (int j=0; j<3; ++j, ++k, ++l) // velocity dim is 3
            {
              // also need the frame rate
              tpl->set(dst.attrib, dst.elems[i], fpreal64(sceneScale * (av[k] + blend * vv[l])), j);
            }
            if (mDim == 4)
            {
              tpl->set(dst.attrib, dst.elems[i], fpreal64(av[k]), 3);
              ++k;
            }
          }
        }
        else
        {
          ...
          for (size_t i=0, k=0, l=0; i<dst.elems.size(); ++i, k+=mDim)
          {
            ...
            {
              // also need the frame rate
              tpl->set(dst.attrib, dst.elems[i], fpreal32(sceneScale * (av[k] + blend * vv[l])), j);
            }
            if (mDim == 4)
            {
              tpl->set(dst.attrib, dst.elems[i], fpreal32(av[k]), 3);
              ++k;
            }
          }
        }
...
#endif
    // No extrapolation for version earlier than 12.0
    {
      //std::cout << "Use current sample" << std::endl;
      return toHoudini(r, sceneScale, dst);
    }
  }
}

bool Attrib::toHoudini(Gto::Reader *r, float sceneScale, HAttrib &dst)
{
#if UT_MAJOR_VERSION_INT >= 12
  ...
  if ((mType == Gto::Float || mType == Gto::Double) &&
      (mDim == 3 || mDim == 4) &&
      (mInterp == "point" || mInterp == "hpoint" || mInterp == "vector"))
  {
  }
  else
  {
  ...
  case Gto::Float:
    {
      ...
      if (isMapped())
      {
        for (size_t i=0, k=0; i<dst.elems.size(); ++i)
        {
          ...
          for (int j=0; j<mDim; ++j, ++k)
          {
            // note for hpoint, should not scale last component
            tpl->set(dst.attrib, dst.elems[i], fpreal32(ptr[k] * (mDim < 3 ? sceneScale : 1.0f)), j);
          }
        }
      }
      else
  ...
  case Gto::Double:
    {
      ...
      if (isMapped())
      {
        for (size_t i=0, k=0; i<dst.elems.size(); ++i)
        {
          ...
          for (int j=0; j<mDim; ++j, ++k)
          {
            tpl->set(dst.attrib, dst.elems[i], fpreal64(ptr[k] * (mDim < 3 ? sceneScale : 1.0f)), j);
          }
        }
      }
      else
  ...
  // Warning: Scene scale not supported
...
void AttribDict::toHoudini(Gto::Reader *r,
                           Gto::Reader *r0, AttribDict *ad0,
                           Gto::Reader *r1, AttribDict *ad1,
                           float blend, float sceneScale)
{
  Attrib *a0, *a1, *v = 0;
  iterator it0, it1, vit;
  ...
  {
    v = &(vit->second.second);
    // float[3] or double[3] non-mapped attribute
    if ((v->getType() != Gto::Double && v->getType() != Gto::Float) || v->getDim() != 3 || v->isMapped())
    {
      v = 0;
    }
  }
  ...
  while (it != end())
  {
    ...
    if (r0 && ad0)
    {
      it0 = ad0->find(it->first);
      if (it0 != ad0->end())
      {
        a0 = &(it0->second.second);
      }
    }
    
    if (r1 && ad1)
    {
      it1 = ad1->find(it->first);
      if (it1 != ad1->end())
      {
        a1 = &(it1->second.second);
      }
    }
    
    it->second.second.toHoudini(r, r0, a0, r1, a1, blend, v, sceneScale, mHA);
    
    ++it;
  }
}

void AttribDict::toHoudini(Gto::Reader *r, float sceneScale)
{
  iterator it = begin();
  while (it != end())
  {
    mHA.attrib = it->second.first;
    it->second.second.toHoudini(r, sceneScale, mHA);
    ++it;
  }
}
